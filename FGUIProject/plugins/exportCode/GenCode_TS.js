...
function genCode(handler) {
    ...
    for (let i = 0; i < classCnt; i++) {
        ...
        writer.writeln();
        writer.writeln("enterAnimation(): void");
        writer.startBlock();
        writer.writeln("fgui.GTween.to2(0, 0, 1, 1, 0.3)");
        writer.writeln("    .setTarget(this)");
        writer.writeln("    .setEase(fgui.EaseType.BackOut)");
        writer.writeln("    .onUpdate((tween) => {");
        ...
        writer.writeln("    .setTarget(this)");
        writer.writeln("    .setEase(fgui.EaseType.BackIn)");
        writer.writeln("    .onUpdate((tween) => {");
        writer.writeln("        this.setScale(tween.value.x, tween.value.y);");
        writer.writeln("    })");
        ...
        for (let j = 0; j < memberCnt; j++) {
            ...
            if (memberInfo.group == 0) {
                ...
                else writer.writeln("this.%s = <%s>(this.getChildAt(%s));", memberInfo.varName, memberInfo.type, memberInfo.index);
            } else if (memberInfo.group == 1) {
                if (getMemberByName) writer.writeln('this.%s = this.getController("%s");', memberInfo.varName, memberInfo.name);
                else writer.writeln("this.%s = this.getControllerAt(%s);", memberInfo.varName, memberInfo.index);
            } else {
                if (getMemberByName) writer.writeln('this.%s = this.getTransition("%s");', memberInfo.varName, memberInfo.name);
                else writer.writeln("this.%s = this.getTransitionAt(%s);", memberInfo.varName, memberInfo.index);
            }
            ...
            if (memberInfo.varName.indexOf("UI_BTN_") == 0) {
                ...
                if (newName) {
                    ...
                    const newName2 = newNames
                        .map((v1) => {
                            let name = v1.toLowerCase();
